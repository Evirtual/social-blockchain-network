...
export function getRpcWsProvider(url: string, chainIdNum: number): WebSocketProvider {
  return new WebSocketProvider(url, chainIdNum);
}
...
export function getRpcWsUrlForChainId(env: EnvMap, chainIdNum: number | null): string {
  if (chainIdNum == null) return "";

  const rpcUrlByChainId: Record<number, string | undefined> = {
    1: env?.VITE_ETH_RPC_WS_URL,
    11155111: env?.VITE_ETH_SEPOLIA_RPC_WS_URL,
    8453: env?.VITE_BASE_RPC_WS_URL,
    84532: env?.VITE_BASE_SEPOLIA_RPC_WS_URL,
    56: env?.VITE_BSC_RPC_WS_URL,
    97: env?.VITE_BSC_TESTNET_RPC_WS_URL,
    31337: env?.VITE_LOCAL_RPC_WS_URL
  };
